...
// VPNSL 1.0.6 combined application + destination routing UI by Sazhaev-IA.
...
private enum class RoutingEditorTab { APPLICATIONS, ROUTES }
...
fun ExceptionsTab(
    ...
    var editorTab by remember { mutableStateOf(RoutingEditorTab.APPLICATIONS) }

    LaunchedEffect(Unit) {
        withContext(Dispatchers.IO) {
            settingsStore.migrateLegacyWhitelistMode()
            // VPNSL 1.0.6 always treats the saved package list as applications
            // whose complete traffic is eligible for the VPN policy.
    ...
    CsqttScreen {
        AppSectionCard(
            ...
            CsqttSegmentedControl(
                options = listOf(
                    RoutingEditorTab.APPLICATIONS to "Приложения",
                    RoutingEditorTab.ROUTES to "Маршруты",
                ),
                selected = editorTab,
            ...
            Text(
                text = if (selectedPackages.isEmpty()) {
                    "Приложения не выбраны: активные IP/CIDR-маршруты применяются ко всем приложениям."
                } else {
                    "Выбрано приложений: ${selectedPackages.size}. Их трафик идёт через VPNSL целиком; правила IP/CIDR могут направлять отдельные адреса обратно в мобильную сеть."
                },
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )

            if (editorTab == RoutingEditorTab.APPLICATIONS) {
                HorizontalDivider(color = MaterialTheme.colorScheme.outlineVariant.copy(alpha = 0.5f))
                CsqttSettingRow(
        ...
        if (editorTab == RoutingEditorTab.ROUTES) {
